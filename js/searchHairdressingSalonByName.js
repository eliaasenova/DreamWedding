    ...
    if (this.readyState == 4 && this.status == 200) {
		...
		for(var i = 0; i < myObj.length; i++)
		{
			if(!isEmpty(myObj))
			{
			    ...
			    document.getElementById("hairdressingSalonsDiv").appendChild(anchor);
		...
		if(isEmpty(myObj))
		{
			...
			document.getElementById("hairdressingSalonsDiv").appendChild(paragraph);
		}
	}
};
xhttp.open("POST", "getSearchedHairdressingSalon.php", true);
xhttp.send();
